...
2. **Implementación**:
   ...
   - Extraer la evaluación de cada candidato a una función que devuelva resultado o causa de rechazo, aplicando el orden: repositorio → SHA → rama → run.id → attempt → par de conclusiones.
   - Registrar causas de rechazo; si no hay candidato utilizable y existen candidatos, devolver `unavailable` con mensaje agregado acotado a 200 caracteres.
